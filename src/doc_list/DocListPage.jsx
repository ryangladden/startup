...
function dateMaxMin2(dates) {
    const all = dates.map(date => date.getTime());
...
export function DocList() {
    ...
    async function getCards() {
        try {
            cardsResponse = await fetch("/api/documnents/list", {method: "get"})
    ...
    React.useEffect(() => {
        setBaseFilter(createFilter(allCards))
    }, [allCards])
    ...
    return (
        <main className='container-fluid'>
            ...
            <div className="container-fluid doc-list">
                ...
                <CardList cards={filterAndSort(cardList)}/>
            </div>
        </main>
    );
}